...
def get_target(path):
    ...
    df = df['prepid :']
    df = df.dropna()
    x = np.unique(df.values).astype(int)
    return x
...
def main():
    ...
    if os.path.exists(path + '/prep_restart.dat'):
        print("loading restart file")
        with open(path + '/prep_restart.dat') as f:
    ...
    url_stem = "https://supercon.nims.go.jp/supercon/preparation?prepid="
    ...
    for i in data_entries:
        ...
        if str(title) == "<title>Preparation</title>":
            count += 1
            try:
            ...
            np.savetxt(path + "/prep_restart.dat", data_entries[count:], fmt="%s")
            break
    ...
    if os.path.exists(path + '/preparation.csv'):
        running_result = pd.read_csv(path + '/preparation.csv', index_col=0)
        result = pd.concat(
    ...
    f = open(str(path) + "/preparation.csv", "w")  # FHSU
    f.write(csv_records)
    f.close()

    if count == len(data_entries):  
        os.remove(path + "/prep_restart.dat")
